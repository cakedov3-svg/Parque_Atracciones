...
public class VistaDepartamentosBuscar extends JFrame implements IGUI {
	
	private JTextField txt;
	private static final String ID_ERROR_TEXT = "El id del evento debe ser un número entero. Vuelva a intenterlo";
	
	public VistaDepartamentosBuscar() {
		super("Buscar un departamento");
		initGUI();
	}
	...
	private void initGUI() {
		...
		botonok.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				Controlador.getInstancia().accion(EventosControlador.DEPARTAMENTOS_BUSCAR, id);
				setVisible(false);
			}
		});
	...
	public void actualizar(int evento, Object datos) {
		if(evento==EventosControlador.DEPARTAMENTOS_BUSCAR_OK) {
			setVisible(false);
			TDepartamento dep = (TDepartamento) datos;
			StringBuilder info = new StringBuilder("Se encontro el departamento con los siguientes datos\n");
			info.append("idDep: ");
			info.append(dep.getidDep()+"\n");
			info.append("Nombre: ");
			info.append(dep.getNombre()+"\n");
			info.append("Activo: ");
			if(dep.isActivo()) {
				info.append("True\n");
			}
			...
			setVisible(false);
			Utils.showErrorMsg("No se encontro el departamento");
		}
	}
